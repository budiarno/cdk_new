...
$(D)/lua: $(D)/bootstrap $(D)/libncurses $(ARCHIVE)/lua-$(LUA_VER).tar.gz
	$(START_BUILD)
	$(REMOVE)/lua-$(LUA_VER)
	$(SILENT)set -e; if [ ! -d $(ARCHIVE)/luaposix.git ]; \
	...
	$(SILENT)set -e; cd $(BUILD_TMP)/lua-$(LUA_VER); \
		for i in \
			lua-$(LUA_VER)-luaposix-$(LUAPOSIX_VER).patch \
		; do \
			echo -e "==> \033[31mApplying Patch:\033[0m $$i"; \
			$(PATCH)/$$i; \
		done; \
...
$(D)/luaexpat: $(D)/bootstrap $(D)/lua $(D)/libexpat $(ARCHIVE)/luaexpat-$(LUAEXPAT_VER).tar.gz
	...
	$(SILENT)set -e; cd $(BUILD_TMP)/luaexpat-$(LUAEXPAT_VER); \
		for i in \
			luaexpat-$(LUAEXPAT_VER).patch \
		; do \
			echo -e "==> \033[31mApplying Patch:\033[0m $$i"; \
			$(PATCH)/$$i; \
		done; \
...
$(D)/lua-feedparser: $(D)/bootstrap $(D)/lua $(D)/luasocket $(D)/luaexpat
	...
	$(SILENT)set -e; if [ -d $(ARCHIVE)/lua-feedparser.git ]; \
	...
	$(SILENT)cp -ra $(ARCHIVE)/lua-feedparser.git $(BUILD_TMP)/lua-feedparser
	$(SILENT)set -e; cd $(BUILD_TMP)/lua-feedparser; \
...
$(D)/libcurl: $(D)/bootstrap $(D)/openssl $(D)/zlib $(ARCHIVE)/curl-$(CURL_VER).tar.bz2
	...
	$(SILENT)cd $(TARGETPREFIX) && rm usr/bin/curl
...
$(D)/librtmpdump: $(D)/bootstrap $(D)/zlib $(D)/openssl
	$(START_BUILD)
	$(REMOVE)/librtmpdump
	$(SILENT)set -e; if [ -d $(ARCHIVE)/rtmpdump.git ]; \
		then cd $(ARCHIVE)/rtmpdump.git; git pull; \
		else cd $(ARCHIVE); git clone git://github.com/oe-alliance/rtmpdump.git rtmpdump.git; \
		fi
	$(SILENT)cp -ra $(ARCHIVE)/rtmpdump.git $(BUILD_TMP)/librtmpdump
	$(SILENT)set -e; cd $(BUILD_TMP)/librtmpdump; \
		for i in \
			rtmpdump-2.4.patch \
		; do \
			echo -e "==> \033[31mApplying Patch:\033[0m $$i"; \
