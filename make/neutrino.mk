...
$(D)/libstb-hal-cst-next-max.do_prepare:
	...
	$(SILENT)set -e; cd $(SOURCE_DIR)/libstb-hal-cst-next-max; \
...
$(D)/libstb-hal-cst-next-max.config.status: | $(NEUTRINO_DEPS)
	...
	cd $(LH_OBJDIR); \
		...
		$(BUILDENV) \
		$(SOURCE_DIR)/libstb-hal-cst-next-max/configure $(CONFIGURE_SILENT) --enable-silent-rules \
			--host=$(TARGET) \
			--build=$(BUILD) \
			--prefix= \
			--with-target=cdk \
...
$(D)/neutrino-mp-cst-next-max.do_prepare: | $(NEUTRINO_DEPS) $(D)/libstb-hal-cst-next-max
	...
	$(SILENT)set -e; cd $(SOURCE_DIR)/neutrino-mp-cst-next-max; \
...
$(D)/libstb-hal-cst-next.do_prepare:
	...
	$(SILENT)set -e; cd $(SOURCE_DIR)/libstb-hal-cst-next; \
...
$(D)/neutrino-mp-cst-next.do_prepare: | $(NEUTRINO_DEPS) $(D)/libstb-hal-cst-next
	...
	$(SILENT)set -e; cd $(SOURCE_DIR)/neutrino-mp-cst-next; \
...
$(D)/neutrino-mp-cst-next-ni.do_prepare: | $(NEUTRINO_DEPS) $(D)/libstb-hal-cst-next
	...
	$(SILENT)set -e; cd $(SOURCE_DIR)/neutrino-mp-cst-next-ni; \
...
$(D)/neutrino-mp-cst-next-ni.config.status:
	...
	cd $(N_OBJDIR); \
		...
		$(BUILDENV) \
		$(SOURCE_DIR)/neutrino-mp-cst-next-ni/configure $(CONFIGURE_SILENT) --enable-silent-rules \
			--build=$(BUILD) \
			--host=$(TARGET) \
			$(N_CONFIG_OPTS) \
			--with-boxtype=$(BOXTYPE) \
...
$(D)/neutrino-hd2.do_prepare: | $(NEUTRINO_DEPS) $(NEUTRINO_DEPS2)
	...
	$(SILENT)set -e; cd $(SOURCE_DIR)/neutrino-hd2; \
...
$(D)/libstb-hal-cst-next-tangos.do_prepare:
	...
	for i in $(NEUTRINO_MP_LIBSTB_CST_NEXT_TANGOS_PATCHES); do \
		echo -e "==> \033[31mApplying Patch:\033[0m $(subst $(PATCHES)/,'',$$i)"; \
		patch -p1 -i $$i; \
	done;
	$(TOUCH)
...
$(D)/neutrino-mp-tangos.do_prepare: | $(NEUTRINO_DEPS) $(D)/libstb-hal-cst-next-tangos
	...
	$(SILENT)set -e; cd $(SOURCE_DIR)/neutrino-mp-tangos; \
